...
class DemoFlowTest < ActionDispatch::IntegrationTest
  ...
  test "full demo loop: register, create repo, and verify metadata" do
    ...
    data = json_response
    assert data["pat"].present?, "PAT should be returned on registration"
    ...
    assert clone_url.include?("my-deploy-tool.git"), "clone_url should include repo name"
    ...
    # Step 5: Star the repo (simulate "agent clones and stars")
  ...
  test "search returns slack-notify as top result for demo query" do
    # Seed the slack-notify repo if not present
  ...
  test "search returns relevant results for multiple demo queries" do
    # Ensure baseline repos exist from seed data
    unless Repo.exists?(owner: "lore-agent", name: "slack-notify")
      disk_path = File.join(@repo_root, "lore-agent", "slack-notify.git")
      User.find_or_create_by!(username: "lore-agent")
      repo = Repo.create!(
  ...
  test "unauthenticated push is rejected" do
    # The Git HTTP auth middleware rejects pushes without auth
    # We can't do a real git push in an integration test, but we can test
    # that POST to git-receive-pack without auth returns 401
    disk_path = File.join(@repo_root, "lore-agent", "test-push.git")
    FileUtils.mkdir_p(disk_path)
    system("git init --bare #{Shellwords.escape(disk_path)} -q 2>/dev/null")

    # This will hit the Grack middleware which checks auth
    # The middleware should return 401 for unauthenticated pushes
    post "/git/lore-agent/test-push.git/git-receive-pack",
      headers: { "Content-Type" => "application/x-git-receive-pack-request" }
    assert_includes [401, 403, 404], response.status,
      "Unauthenticated push should be rejected, got #{response.status}"
  end
end
